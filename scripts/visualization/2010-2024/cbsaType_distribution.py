...
df_primary = df_primary[
    (df_primary['DisappearanceDate'] > pd.to_datetime('1999-12-31')) &
    (df_primary['DisappearanceDate'] < pd.to_datetime('2025-01-01'))
]

def plot_cbsa_type_distribution(df):
    """
    Plots the distribution of values in the 'CBSA Type' column,
    including entries that are not assigned to any CBSA (NaN),
    with individual bar colors and percentage annotations.
    """
    ...
    plt.title(
        f"Distribution of NamUS Missing Persons Cases by CBSA Type (2000–2024)",
        fontsize=32
    )
    plt.xlabel("CBSA Type", fontsize=32)
