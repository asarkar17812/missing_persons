...
#          color='darkgreen', linewidth=2, label='Cumulative NamUS Missing Persons Cases (2000–2024)')


# # --- Formatting
# plt.title('Cumulative NamUS Missing Persons Cases by Month Start (2000–2024)', fontsize=44)
# plt.xlabel('Year', fontsize=40)
# plt.ylabel('Cumulative NamUS Missing Persons Cases', fontsize=38)
# plt.grid(False)

# # Format x-axis ticks for Januarys only
# jan_ticks = january_points.index
# plt.xticks(ticks=jan_ticks, labels=[date.strftime('%Y') for date in jan_ticks], fontsize=28, rotation=55)
# y_ticks=[12000, cumulative_disappearances['2000-01-01'],14000, 16000, 18000, 20000, 22000, 24000]
# plt.yticks(y_ticks, fontsize=28)
# plt.ylim(y_ticks[0] + .5)
# plt.xlim(all_months[-185], all_months[-1])
...
years = list(range(2000, 2025))  # 2025 excluded
...
plt.title(r'Scaling Exponent ($\beta$) of Total NamUS Missing Person Cases vs County Population (2000–2024)', fontsize=36)
plt.xlabel('Cumulative NamUS Cases by Year', fontsize=34)
plt.ylabel(r'Estimated Scaling Exponent Value ($\beta$)', fontsize=34)
plt.xticks(years_to_plot, rotation=65, fontsize=22)
...
def plot_r2_timeseries(years, r2_values):
    ...
    plt.title(r'Time Series of $R^2$ Values (2000–2024) for $\beta$ for Counties', fontsize=32)
